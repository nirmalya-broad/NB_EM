...
*/
{
    ...
    double ret_val = 100.0*pow(y - x*x,2) + pow(1 - x,2);
    std::cout << "ret_val from rosen: " << rosen << "\n";
    return ret_val;
}
...
*/
    ...
    starting_point = {0.5, 0.5}; // Start with a valid point inside the constraint box.
    std::cout << "starting point: " << starting_point << "\n";

    const column_vector x_lower1 = {0.1, 0.1};
    const column_vector x_upper1 = {0.8, 0.8};

    find_min_box_constrained(lbfgs_search_strategy(10),  
                             objective_delta_stop_strategy(1e-9),  
                             rosen, rosen_derivative, starting_point, 
                             x_lower1, x_upper1);
